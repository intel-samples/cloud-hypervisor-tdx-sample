...
test_features="--features kvm,tdx"
build_features="kvm,tdx"
...
time cargo nextest run $test_features --retries 3 --no-fail-fast --no-tests=pass --test-threads=$(($(nproc) / 4)) "common_tdx::$test_filter" -- ${test_binary_args[*]}
RES=$?
